...
@app.route('/api/devices/<int:device_id>/sync-os-ip', methods=['POST'])
def sync_os_ip(device_id):
    """通过 BMC 同步单台设备的 OS IP"""
    from collector_service import sync_os_ip_from_bmc
    result = sync_os_ip_from_bmc(device_id, app)
    success = '已同步' in result or '未获取到' in result
    return jsonify({
        'message': result,
        'success': success,
...
@app.route('/api/devices/sync-os-ip-all', methods=['POST'])
def sync_os_ip_all():
    """批量同步所有设备的 OS IP"""
    from collector_service import sync_os_ip_for_all
    result = sync_os_ip_for_all(app)
    return jsonify({
        'message': f"批量同步完成: 成功 {result['success']} 台, 失败 {result['failed']} 台",
