    ...
    def get_network_info(self) -> List[Dict]:
        ...
        try:
            ...
            for sys_member in systems_data.get('Members', []):
                ...
                # 路径1: Systems/*/EthernetInterfaces
                ...
                if eth_resp.status_code == 200:
                    ...
                    for eth_member in eth_data.get('Members', []):
                        ...
                        if not eth_url:
                            continue
                        eth_detail = self.session.get(f"{self.base_url}{eth_url}", timeout=30)
                        if eth_detail.status_code != 200:
                            continue
                        ed = eth_detail.json()
                        
                        # 获取 IP 地址
                        ip_addresses = []
                        if ed.get('IPv4Addresses'):
                            for ip_info in ed.get('IPv4Addresses', []):
                                if ip_info.get('Address'):
                                    ip_addresses.append(ip_info.get('Address'))
                        
                        network_list.append({
                            'name': ed.get('Name', ed.get('Id', '')),
                            'description': ed.get('Description', ''),
                            'mac_address': ed.get('MACAddress', ''),
                            'ip_addresses': ip_addresses,
                            'status': ed.get('Status', {}).get('State', ''),
                        })
                
                # 路径2: Systems/*/NetworkInterfaces (某些厂商使用这个路径)
                net_resp = self.session.get(
                    f"{self.base_url}{system_url}/NetworkInterfaces", timeout=30)
                if net_resp.status_code == 200:
                    net_data = net_resp.json()
                    for net_member in net_data.get('Members', []):
                        net_url = net_member.get('@odata.id', '')
                        if not net_url:
                            continue
                        net_detail = self.session.get(f"{self.base_url}{net_url}", timeout=30)
                        if net_detail.status_code != 200:
                            continue
                        nd = net_detail.json()
                        
                        ip_addresses = []
                        if nd.get('IPv4Addresses'):
                            for ip_info in nd.get('IPv4Addresses', []):
                                if ip_info.get('Address'):
                                    ip_addresses.append(ip_info.get('Address'))
                        
                        # 去重
                        exists = any(n.get('mac_address') == nd.get('MACAddress') for n in network_list)
                        if not exists:
                            network_list.append({
                                'name': nd.get('Name', nd.get('Id', '')),
                                'description': nd.get('Description', ''),
                                'mac_address': nd.get('MACAddress', ''),
                                'ip_addresses': ip_addresses,
                                'status': nd.get('Status', {}).get('State', ''),
                            })
        except Exception as e:
            print(f"  获取网络信息异常: {str(e)}")
        
        return network_list

    def get_os_ip(self) -> str:
        """从 BMC 获取操作系统的主 IP 地址（排除 BMC 自身 IP）"""
        ...
        for iface in network_info:
            for ip in iface.get('ip_addresses', []):
                # 排除 BMC IP 和回环地址
                if ip != bmc_ip and not ip.startswith('127.') and not ip.startswith('::1'):
                    return ip
