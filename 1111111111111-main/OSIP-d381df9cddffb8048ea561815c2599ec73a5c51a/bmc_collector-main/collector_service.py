...
def _collect_one(device: Device) -> str:
    ...
    # ---------- 通过 BMC 获取 OS IP ----------
    os_ip_from_bmc = info.get('os_ip', '')
    if os_ip_from_bmc and not device.os_ip:
        device.os_ip = os_ip_from_bmc
        print(f"  从 BMC 获取 OS IP: {os_ip_from_bmc}")
...
def sync_os_ip_from_bmc(device_id: int, app=None) -> str:
    """通过 BMC 同步 OS IP（强制更新，即使已有OS IP也会覆盖）"""
    ...
    with _app.app_context():
        ...
        try:
            collector.connect_redfish()
            os_ip = collector.get_os_ip()
            collector.disconnect_redfish()
            
            if os_ip:
                old_os_ip = device.os_ip
                device.os_ip = os_ip
                db.session.commit()
                return f"OS IP 已同步: {'无' if not old_os_ip else old_os_ip} -> {os_ip}"
            else:
                return '从BMC未获取到OS IP'
        except Exception as e:
            return f"同步失败: {str(e)}"


def sync_os_ip_for_all(app=None) -> dict:
    """批量同步所有设备的 OS IP"""
    from app import create_app
    _app = app or create_app()
    with _app.app_context():
        ...
        for device in devices:
            result = sync_os_ip_from_bmc(device.id, _app)
            if '已同步' in result or '未获取到' in result:
                results['success'] += 1
            else:
                results['failed'] += 1
            results['details'][device.bmc_ip] = result
